...
const WeatherIcon = ({ code, size = 24 }) => {
  ...
  if (category === 'storm')
    return (
      <svg width={s} height={s} viewBox="0 0 24 24" fill="none">
        <path d="M19 16.9A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25" stroke="#4A8C5A" strokeWidth="2" strokeLinecap="round"/>
...
function Weather() {
  ...
  return (
    <div className="min-h-screen bg-[#F5FFFB] font-sans">
      <div className="px-6 pt-8 pb-4 max-w-3xl mx-auto">
        <Link to="/" className="inline-flex items-center gap-2 text-sm text-[#4A8C5A] hover:text-[#1B4D2E] transition-colors">
      ...
      <div className="px-6 pb-16 max-w-3xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-medium text-[#1B4D2E] tracking-tight mb-1">Weather Forecast</h1>
          <p className="text-sm text-[#4A8C5A]">Hyperlocal forecasts tailored for your farm</p>
        </div>

        {/* Search */}
        <form onSubmit={handleSearch} className="flex gap-2 mb-8">
          <div className="relative flex-1">
            <svg className="absolute left-4 top-1/2 -translate-y-1/2 text-[#4A8C5A]" width="16" height="16" viewBox="0 0 24 24" fill="none">
            ...
            <input
              ...
              className="w-full bg-white border border-[#A8D5A8]/50 rounded-full pl-10 pr-4 py-3 text-sm text-[#1B4D2E] placeholder-[#4A8C5A]/50 focus:outline-none focus:border-[#6BA876] transition-colors"
            />
          </div>
          <button
            type="submit"
            className="bg-[#1B4D2E] text-[#D4EDD4] text-sm px-6 py-3 rounded-full hover:bg-[#2D6B44] active:scale-95 transition-all duration-200"
          >
            Search
          </button>
        </form>
        ...
        {current && !loading && (
          <>
            <div className="bg-white border border-[#A8D5A8]/40 rounded-3xl p-6 mb-4">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <p className="text-xs text-[#4A8C5A] mb-1 uppercase tracking-widest">Current weather</p>
                  <h2 className="text-xl font-medium text-[#1B4D2E]">{locationName}</h2>
                  <p className="text-sm text-[#4A8C5A] mt-0.5">
              ...
              <div className="flex items-end gap-2 mb-6">
                <span className="text-7xl font-medium text-[#1B4D2E] leading-none">
                  {Math.round(current.temperature_2m)}°
                </span>
                <span className="text-2xl text-[#4A8C5A] mb-2">C</span>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={label} className="bg-[#F5FFFB] rounded-2xl p-3">
                    <p className="text-xs text-[#4A8C5A] mb-1">{label}</p>
                    <p className="text-sm font-medium text-[#1B4D2E]">{value}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* AI Farming Tip */}
            <div className="bg-[#D4EDD4] border border-[#A8D5A8] rounded-2xl p-4 mb-4 flex items-start gap-3">
              <div className="w-7 h-7 bg-[#6BA876] rounded-full flex items-center justify-center shrink-0 mt-0.5">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                  <path d="M12 22V12M12 12C12 7 7 4 2 5c0 5 3 9 10 7M12 12c0-5 5-8 10-7-1 5-4 9-10 7" stroke="#D4EDD4" strokeWidth="2" strokeLinecap="round"/>
                </svg>
              </div>
              <div className="flex-1">
                <p className="text-xs font-medium uppercase tracking-widest mb-3 text-[#6BA876]">AI Farming Tips</p>
                <ul className="space-y-2 pl-0 list-none">
                  {aiTips && Array.isArray(aiTips) && aiTips.length > 0 ? (
                    aiTips.map((tip, idx) => (
                      <li key={idx} className="flex gap-2 items-start text-sm text-[#2D6B44] leading-relaxed">
                        <span className="text-[#6BA876] font-bold text-lg mt-[-6px]">•</span>
                        <span>{tip}</span>
                      </li>
                    ))
                  ) : (
                    <li className="text-sm text-[#2D6B44]">Check local conditions before major farming activities.</li>
                  )}
                </ul>
              </div>
            </div>
            ...
            {/* 5-Day Forecast */}
            <div className="bg-white border border-[#A8D5A8]/40 rounded-3xl p-6">
              <p className="text-xs text-[#4A8C5A] uppercase tracking-widest mb-4">5-day forecast</p>
              <div className="flex flex-col gap-3">
                {forecast.time?.slice(1, 6).map((date, i) => (
                  <div key={date} className="flex items-center justify-between py-2 border-b border-[#A8D5A8]/20 last:border-0">
                    <span className="text-sm font-medium text-[#1B4D2E] w-12">{getDayName(date)}</span>
                    <WeatherIcon code={forecast.weather_code[i + 1]} size={20}/>
                    <span className="text-xs text-[#4A8C5A] capitalize flex-1 text-center">
